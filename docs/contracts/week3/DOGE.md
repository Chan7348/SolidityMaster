...
```solidity
function MAX_DOGEs() external view returns (uint256)
```
...
```solidity
function claimETH() external nonpayable
```
...
```solidity
function fuckYouAll() external nonpayable
```
...
```solidity
event TokenClaim(address claimer, uint256 amount)
```
